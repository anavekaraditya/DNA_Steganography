...
def subBytes(A):
  s_box = np.load('TableLookup/s_box.npy')
...
def invSubBytes(A):
  inv_s_box = np.load('TableLookup/inv_s_box.npy')
  B = np.zeros((4,4),dtype=int)
  for row in range(4):
    for col in range(4):
      sub_row, sub_col = A[row,col]//16, A[row,col]%16
...
def mixCol(A):
  e_table = np.load('TableLookup/E_Table.npy')
  B = np.zeros((4,4),dtype=int)
  for row in range(4):
    for col in range(4):
      sub_row , sub_col = A[row,col]//16,A[row,col]%16
...
def invMixCol(A):
  l_table = np.load('TableLookup/L_Table.npy')
  B = np.zeros((4,4),dtype=int)
  for row in range(4):
    for col in range(4):
      sub_row , sub_col = A[row,col]//16,A[row,col]%16
